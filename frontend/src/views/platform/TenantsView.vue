...
import { createTenant, createTenantUser, establishTenantAccess, listTenants } from "../../api/tenants";
...
async function saveUser() { saving.value = true; try { await createTenantUser(selected.value.id, userForm.value); success.value = `User created for ${selected.value.name}.`; showUser.value = false; userForm.value = { email: "", first_name: "", last_name: "", password: "", role_code: "school_admin" }; } catch (e) { error.value = getApiError(e); } finally { saving.value = false; } }
async function accessTenant(tenant) { const reason = window.prompt("Reason for audited tenant access:", "Support and configuration verification"); if (!reason) return; try { const result = await establishTenantAccess(tenant.id, reason); localStorage.setItem("shulelink_access_token", result.access_token); window.location.href = `http://${tenant.host}:5173/school`; } catch (e) { error.value = getApiError(e); } }
onMounted(load);
</script>
...
<template>
  <div class="page-wrap">
    <div class="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-4">
      <div><span class="eyebrow">PLATFORM / SCHOOLS</span><h1 class="h3 fw-bold mt-2 mb-1">Schools</h1><p class="text-muted mb-0">Create and manage ShuleLink tenant environments.</p></div>
      <button class="btn btn-primary" @click="showCreate = true"><i class="bi bi-plus-lg me-2"></i>New school</button>
    </div>
    <div v-if="error" class="alert alert-danger">{{ error }}</div><div v-if="success" class="alert alert-success">{{ success }}</div>
    <div class="card border-0 shadow-sm overflow-hidden">
      <div class="table-responsive"><table class="table align-middle mb-0"><thead><tr><th>School</th><th>Domain</th><th>Database</th><th>Status</th><th class="text-end">Actions</th></tr></thead>
      <tbody><tr v-if="loading"><td colspan="5" class="text-center py-5"><span class="spinner-border spinner-border-sm"></span></td></tr><tr v-else-if="!tenants.length"><td colspan="5" class="text-center py-5 text-muted">No schools yet. Create your first school.</td></tr>
      <tr v-for="tenant in tenants" :key="tenant.id"><td><div class="fw-semibold">{{ tenant.name }}</div><div class="tiny text-muted">{{ tenant.slug }}</div></td><td><code>{{ tenant.host }}</code></td><td><span class="badge rounded-pill text-bg-light border">{{ tenant.database_mode }}</span></td><td><span class="status-dot" :class="tenant.status"></span>{{ tenant.status }}</td><td class="text-end"><div class="btn-group"><button class="btn btn-sm btn-light border" @click="openUser(tenant)"><i class="bi bi-person-plus me-1"></i>User</button><button class="btn btn-sm btn-light border" @click="accessTenant(tenant)"><i class="bi bi-box-arrow-in-right me-1"></i>Access</button></div></td></tr></tbody></table></div>
    </div>
