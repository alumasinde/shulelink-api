...
const assignmentSubjects = computed(() => assignmentClassSubjects.value.map(cs => data.value.subjects.find(s => String(s.id) === String(cs.subject_id))).filter(Boolean))
...
function resetAssignment() { assignmentForm.value = { teacher_id:null, academic_year_id:null, academic_term_id:null, class_level_id:null, stream_id:null, subject_id:null } }
async function addTeacher() { await run(async () => { await teachers.create(teacherForm.value); resetTeacher() }) }
async function addAssignment() {
  if (!assignmentReady.value) { error.value = 'Select a teacher, academic year, term, class and a subject assigned to that class.'; return }
  const validSubject = assignmentSubjects.value.some(s => String(s.id) === String(assignmentForm.value.subject_id))
  if (!validSubject) { error.value = 'The selected subject is not assigned to this class. Choose one of the available class subjects.'; return }
