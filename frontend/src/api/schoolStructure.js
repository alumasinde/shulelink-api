...
const resource = (name) => ({
  ...
  update: async (id, payload) => (await api.patch(`/school-structure/${name}/${id}`, payload)).data,
})
...
export const classSubjects = {
  ...
  update: async (id, payload) => (await api.patch(`/school-structure/class-subjects/${id}`, payload)).data,
  remove: async (id) => (await api.delete(`/school-structure/class-subjects/${id}`)).data,
}
