...
CREATE TABLE IF NOT EXISTS attendance_policies (
    ...
    KEY ix_attendance_policy_scope (tenant_id, scope_type, scope_id, enabled, effective_from, effective_to),
    CONSTRAINT fk_attendance_policy_tenant FOREIGN KEY (tenant_id) REFERENCES tenants(id) ON DELETE CASCADE,
    CONSTRAINT chk_attendance_policy_scope CHECK ((scope_type = 'school' AND scope_id IS NULL) OR (scope_type <> 'school' AND scope_id IS NOT NULL)),
...
CREATE TABLE IF NOT EXISTS attendance_records (
    ...
    CONSTRAINT chk_attendance_record_source CHECK (source <> ''),
...
CREATE TABLE IF NOT EXISTS attendance_events (
    ...
    CONSTRAINT chk_attendance_event_type CHECK (event_type <> ''),
    CONSTRAINT chk_attendance_event_source CHECK (source <> '')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
CREATE TABLE IF NOT EXISTS attendance_devices (
    ...
    CONSTRAINT chk_attendance_device_type CHECK (device_type <> '')
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4 COLLATE=utf8mb4_unicode_ci;
...
WHERE r.code = 'school_admin' AND p.code IN ('attendance.read','attendance.mark','attendance.edit','attendance.correct','attendance.approve','attendance.manage','attendance.reports','attendance.export');

INSERT IGNORE INTO tenant_role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM tenant_roles r JOIN tenant_permissions p ON p.tenant_id = r.tenant_id
WHERE r.code = 'registrar' AND p.code IN ('attendance.read','attendance.mark','attendance.correct','attendance.reports','attendance.export');

INSERT IGNORE INTO tenant_role_permissions (role_id, permission_id)
SELECT r.id, p.id FROM tenant_roles r JOIN tenant_permissions p ON p.tenant_id = r.tenant_id
WHERE r.code = 'teacher' AND p.code IN ('attendance.read','attendance.mark','attendance.correct');
