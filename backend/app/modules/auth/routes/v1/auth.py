...
from app.modules.auth.hardening_service import begin_mfa_enrollment, confirm_password_reset, disable_mfa, mfa_status, verify_mfa_challenge, verify_mfa_enrollment
from app.modules.auth.mfa_reauth import verify_mfa_disable_credentials
from app.modules.auth.password_reset import request_password_reset_scoped
...
async def _login(request,response,payload,user_type,tenant_id=None):
    try:
        result=await login_user(payload.email,payload.password,user_type,tenant_id); logger.info("authentication succeeded",extra={"event":"authn_login_success","method":request.method,"path":request.url.path,"status_code":200,"user_type":user_type,"tenant_id":str(tenant_id) if tenant_id else None})
        if settings.auth_cookie_mode and not result["mfa_required"]: set_auth_cookies(response,result["access_token"],result["refresh_token"]); result={**result,"access_token":None,"refresh_token":None}
        return TokenResponse(**result)
    except Exception: logger.warning("authentication failed",extra={"event":"authn_login_failure","method":request.method,"path":request.url.path,"status_code":401,"user_type":user_type,"tenant_id":str(tenant_id) if tenant_id else None}); raise
@router.get("/csrf")
...
async def password_reset_request(request:Request,payload:PasswordResetRequest):
    tenant_id=None if payload.user_type=="platform" else await get_tenant_id_from_host(request); token=await request_password_reset_scoped(payload.user_type,payload.email,tenant_id); data={"message":"If the account exists, password reset instructions have been sent."}
