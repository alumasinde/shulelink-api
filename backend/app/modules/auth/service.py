        ...
        try:
            ...
            async with conn.cursor() as cur:
                ...
                if user_type == "platform":
                    await cur.execute("UPDATE platform_users SET last_login_at=%s WHERE id=%s", (now, str(user[0])))
                else:
                    await cur.execute("UPDATE tenant_users SET last_login_at=%s WHERE id=%s", (now, str(user[0])))
...
async def refresh_session(refresh_token: str):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            if session[1] == "platform":
                await cur.execute("SELECT status FROM platform_users WHERE id=%s", (str(session[2]),))
            else:
                await cur.execute("SELECT status FROM tenant_users WHERE id=%s", (str(session[2]),))
            user = await cur.fetchone()
