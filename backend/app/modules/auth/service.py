        ...
        try:
            ...
            async with conn.cursor() as cur:
                if user_type == "tenant":
                    ...
                    await cur.execute(
                        "SELECT u.id,u.email,u.first_name,u.last_name,u.password_hash,u.status FROM tenant_users u "
                        "JOIN tenant_memberships m ON m.tenant_user_id=u.id "
                        "WHERE m.tenant_id=%s AND m.status='active' AND (u.email=%s OR u.login_identifier=%s) LIMIT 1",
                        (str(tenant_id), normalized, normalized),
                    )
                else:
                    await cur.execute(
                        "SELECT id,email,first_name,last_name,password_hash,status FROM platform_users WHERE email=%s LIMIT 1",
                        (normalized,),
                    )
                ...
                await cur.execute(
                    "SELECT 1 FROM mfa_factors WHERE user_type=%s AND user_id=%s AND factor_type='totp' AND enabled=1 LIMIT 1",
                    (user_type, str(user[0])),
                )
                ...
                await cur.execute(
                    "INSERT INTO identity_audit_log (actor_type,actor_id,tenant_id,action,target_type,target_id) VALUES (%s,%s,%s,'auth.login',%s,%s)",
                    (user_type, str(user[0]), str(tenant_id) if tenant_id else None, user_type, str(user[0])),
                )
                await conn.commit()
                await clear_login_throttle(user_type, normalized)
...
async def refresh_session(refresh_token: str):
    """Rotate a refresh token under row lock so concurrent refreshes cannot both succeed."""
    pool = get_pool()
    ...
    async with pool.acquire() as conn:
        try:
            ...
            async with conn.cursor() as cur:
                await cur.execute(
                    "SELECT id,user_type,user_id,tenant_id,tenant_access_session_id,expires_at,revoked_at "
                    "FROM auth_sessions WHERE refresh_token_hash=%s LIMIT 1 FOR UPDATE",
                    (token_hash,),
                )
                ...
                if session[4]:
                    await cur.execute(
                        "SELECT expires_at,revoked_at,tenant_id,platform_user_id FROM tenant_access_sessions WHERE id=%s LIMIT 1",
                        (str(session[4]),),
                    )
                    access_session = await cur.fetchone()
                    if (
                        not access_session
                        or access_session[1] is not None
                        or access_session[0] <= now
                        or str(access_session[3]) != str(session[2])
                        or str(access_session[2]) != str(session[3])
                    ):
                        await conn.rollback()
                ...
                await cur.execute(
                    "UPDATE auth_sessions SET refresh_token_hash=%s,expires_at=%s,last_used_at=%s "
                    "WHERE id=%s AND refresh_token_hash=%s AND revoked_at IS NULL",
                    (new_hash, new_expires.replace(tzinfo=None), now, str(session[0]), token_hash),
                )
                if cur.rowcount != 1:
                    await conn.rollback()
                ...
                access, access_expires = create_access_token(
                    user_id=str(session[2]),
                    user_type=session[1],
                    tenant_id=str(session[3]) if session[3] else None,
                    session_id=str(session[0]),
                    access_session_id=str(session[4]) if session[4] else None,
                )
                await conn.commit()
...
async def logout_session(refresh_token: str | None, session_id: UUID | None):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            if refresh_token:
                await cur.execute(
                    "UPDATE auth_sessions SET revoked_at=%s WHERE refresh_token_hash=%s AND revoked_at IS NULL",
                    (now, hash_token(refresh_token)),
                )
            elif session_id:
                await cur.execute(
