...
async def _platform_settings():
    central = get_central_pool(); result = {}
    async with central.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            for key, value, value_type in await cur.fetchall(): result[key] = _decode(value, value_type)
    return result


async def effective_academic_setting(tenant_id: UUID, key: str, default=None):
    settings = await _platform_settings(); tenant = get_pool()
    async with tenant.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute('SELECT setting_value,value_type FROM school_settings WHERE tenant_id=%s AND setting_key=%s', (str(tenant_id), key))
            row = await cur.fetchone()
    if row: return _decode(row[0], row[1])
    return settings.get(key, default)


async def effective_academic_configuration(tenant_id: UUID):
    platform_settings = await _platform_settings(); tenant = get_pool(); overrides = {}
    async with tenant.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            for key, value, value_type in await cur.fetchall(): overrides[key] = _decode(value, value_type)
            ...
            reference = {}
            for category, value_key, label, sort_order in await cur.fetchall(): reference.setdefault(category, []).append({'key': value_key, 'label': label, 'sort_order': sort_order})
            await cur.execute('''SELECT f.code,f.name,v.code,v.name FROM curriculum_frameworks f LEFT JOIN curriculum_versions v ON v.framework_id=f.id AND v.tenant_id=f.tenant_id AND v.is_active=1 WHERE f.tenant_id=%s AND f.is_active=1 ORDER BY f.is_default DESC,f.name,v.effective_from DESC''', (str(tenant_id),))
    ...
    return {'settings': {**platform_settings, **overrides}, 'reference_values': reference, 'curriculum': {'frameworks': curriculum, 'education_levels': levels, 'grades': grades}}
