...
from app.modules.academics.timetable_service import list_timetable, create_timetable, delete_timetable
...
from app.modules.academics.configuration import effective_academic_configuration, effective_academic_setting
...
async def configuration(tenant_id=Depends(read)): return await effective_academic_configuration(tenant_id)
@router.get('/teachers',response_model=list[TeacherResponse])
async def teachers(tenant_id=Depends(teacher_read), status: str|None=Query(None,max_length=40), search: str|None=Query(None,max_length=100)): return await list_teachers(tenant_id,status,search)
...
async def bulk_update_teachers_route(payload:BulkTeacherUpdate,tenant_id=Depends(teacher_manage)): return await bulk_update_teachers(tenant_id,payload.teacher_ids,payload.model_dump(exclude={'teacher_ids'}, exclude_none=True))
@router.put('/teachers/bulk-subjects')
async def bulk_teacher_subjects_route(payload:BulkTeacherSubjects,tenant_id=Depends(teacher_manage)): return await bulk_replace_teacher_subjects(tenant_id,payload.teacher_ids,payload.subject_ids)
...
async def generate(payload:GenerateRequest,tenant_id=Depends(tt_generate)):
    data=payload.model_dump()
    if data.get('lessons_per_week') is None: data['lessons_per_week']=await effective_academic_setting(tenant_id,'academic.default_lessons_per_week',3)
    return await generate_timetable(tenant_id,data)
