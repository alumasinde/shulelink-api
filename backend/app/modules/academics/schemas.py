...
class TeacherCreate(BaseModel):
    ...
    teacher_number: str = Field(min_length=1, max_length=80)
    first_name: str = Field(min_length=1, max_length=100)
    middle_name: str | None = Field(default=None, max_length=100)
    last_name: str = Field(min_length=1, max_length=100)
    gender: str = Field(default='unspecified', min_length=1, max_length=30)
...
class TeacherUpdate(BaseModel):
    tenant_user_id: UUID | None = None
    teacher_number: str | None = Field(default=None, min_length=1, max_length=80)
    ...
    email: str | None = Field(default=None, max_length=190)
    department_id: UUID | None = None
    subject_ids: list[UUID] | None = Field(default=None)
    employment_type: str | None = Field(default=None, max_length=60)
    employment_date: date | None = None
...
class BulkTeacherSubjects(BaseModel):
    teacher_ids: list[UUID] = Field(min_length=1, max_length=500)
    subject_ids: list[UUID] = Field(min_length=0)
...
class PeriodCreate(BaseModel):
    ...
    end_time: time
    is_break: bool = False
    sort_order: int = Field(ge=1, le=1000)
    is_active: bool = True
...
class TimetableCreate(BaseModel):
    ...
    duration_periods: int = Field(default=1, ge=1, le=12)
...
class GenerateRequest(BaseModel):
    ...
    stream_id: UUID | None = None
    lessons_per_week: int | None = Field(default=None, ge=1, le=40)
...
class EffectiveAcademicSettingsResponse(BaseModel):
    settings: dict[str, object]
    reference_values: dict[str, list[dict]]
