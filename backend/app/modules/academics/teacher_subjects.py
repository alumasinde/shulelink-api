...
async def _get_setting(cur, key, default=None):
...
async def replace_teacher_subjects(tenant_id: UUID, teacher_id: UUID, subject_ids: list[UUID]):
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            max_subjects = await _get_setting(cur, 'academic.default_teacher_subject_limit', None)
            ...
            if subject_ids:
                ...
                enforce_department = await _get_setting(cur, 'academic.enforce_department_subject_matching', False)
