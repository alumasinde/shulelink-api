...
FIELDS={
 ...
 'subjects':{'department_id','code','name','short_name','subject_type','learning_area_id','is_active'},
}
    ...
    async with pool.acquire() as conn:
        try:
            ...
            async with conn.cursor() as cur:
                ...
                columns=[d[0] for d in cur.description]; current=dict(zip(columns,row)); final={**current,**data}
                if table=='academic_years' and final.get('end_date')<=final.get('start_date'): raise HTTPException(422,'end_date must be after start_date')
                if table=='academic_terms':
                    ...
                    if final.get('end_date')<=final.get('start_date'): raise HTTPException(422,'end_date must be after start_date')
                    if final['start_date']<academic_year[0] or final['end_date']>academic_year[1]: raise HTTPException(422,'Academic term dates must fall within the selected academic year')
                if table=='streams':
                ...
                if table=='subjects':
                    ...
                    if final.get('learning_area_id'):
                        await cur.execute('SELECT id FROM learning_areas WHERE id=%s AND tenant_id=%s',(str(final['learning_area_id']),str(tenant_id)))
                        if not await cur.fetchone(): raise HTTPException(404,'Learning area not found')
                ...
                try: await cur.execute(f'UPDATE {table} SET {assignments} WHERE id=%s AND tenant_id=%s',[*[_value(v) for v in data.values()],str(item_id),str(tenant_id)])
...
async def update_class_subject(tenant_id,item_id,is_compulsory,**changes):
    pool=get_pool()
    fields={'is_compulsory':bool(is_compulsory)}
    for key in ('curriculum_version_id','grade_id','requirement_type','weekly_periods'):
        if key in changes: fields[key]=changes[key]
    assignments=','.join(f'{k}=%s' for k in fields)
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(f'UPDATE class_subjects SET {assignments} WHERE id=%s AND tenant_id=%s',[*[_value(v) for v in fields.values()],str(item_id),str(tenant_id)])
            if cur.rowcount==0: raise HTTPException(404,'Class subject assignment not found')
            await cur.execute('SELECT id,class_level_id,subject_id,is_compulsory,curriculum_version_id,grade_id,requirement_type,weekly_periods FROM class_subjects WHERE id=%s AND tenant_id=%s',(str(item_id),str(tenant_id)))
            row=await cur.fetchone()
