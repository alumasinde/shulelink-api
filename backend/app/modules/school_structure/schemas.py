...
class TermCreate(BaseModel):
    academic_year_id: UUID; name: str=Field(min_length=2,max_length=80); term_number: int=Field(ge=1,le=12); start_date: date; end_date: date; is_current: bool=False
    @field_validator('end_date')
    @classmethod
    def valid_range(cls,v,info):
        if info.data.get('start_date') and v<=info.data['start_date']: raise ValueError('end_date must be after start_date')
...
class TermUpdate(BaseModel):
    academic_year_id: UUID|None=None; name: str|None=Field(default=None,min_length=2,max_length=80); term_number: int|None=Field(default=None,ge=1,le=12); start_date: date|None=None; end_date: date|None=None; is_current: bool|None=None; is_active: bool|None=None
    @model_validator(mode='after')
    def valid_range(self):
        if self.start_date and self.end_date and self.end_date<=self.start_date: raise ValueError('end_date must be after start_date')
        return self
...
class SubjectCreate(BaseModel):
    department_id: UUID|None=None; code: str=Field(min_length=1,max_length=40); name: str=Field(min_length=2,max_length=120); short_name: str|None=None; subject_type: str=Field(default='core',min_length=1,max_length=60); learning_area_id: UUID|None=None
class SubjectUpdate(BaseModel):
    department_id: UUID|None=None; code: str|None=Field(default=None,min_length=1,max_length=40); name: str|None=Field(default=None,min_length=2,max_length=120); short_name: str|None=None; subject_type: str|None=Field(default=None,min_length=1,max_length=60); learning_area_id: UUID|None=None; is_active: bool|None=None
class SubjectResponse(SubjectCreate): id: UUID; is_active: bool

class ClassSubjectCreate(BaseModel): class_level_id: UUID; subject_id: UUID; is_compulsory: bool=True; curriculum_version_id: UUID|None=None; grade_id: UUID|None=None; requirement_type: str='required'; weekly_periods: float|None=Field(default=None,ge=0)
class ClassSubjectUpdate(BaseModel): is_compulsory: bool; curriculum_version_id: UUID|None=None; grade_id: UUID|None=None; requirement_type: str='required'; weekly_periods: float|None=Field(default=None,ge=0)
class ClassSubjectResponse(ClassSubjectCreate): id: UUID

class SettingUpdate(BaseModel): value: str|None=None; value_type: str=Field(default='string',min_length=1,max_length=30)
