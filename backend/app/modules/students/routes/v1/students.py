...
async def require_read(tenant_id: UUID = Depends(require_tenant_permission("students.read"))): return tenant_id
async def require_manage(tenant_id: UUID = Depends(require_tenant_permission("students.manage"))): return tenant_id
...
async def link_guardian(student_id: UUID, payload: StudentGuardianCreate, tenant_id=Depends(require_manage)):
    await add_student_guardian(tenant_id, student_id, payload.model_dump())
