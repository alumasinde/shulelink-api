...
PHOTO_URL_RE = re.compile(r"^/api/v1/students/[0-9a-fA-F-]{36}/photo$")
...
def validate_photo_url(value: str | None) -> str | None:
    value = clean(value)
    if value and not PHOTO_URL_RE.fullmatch(value):
        raise ValueError("photo_url must be a server-managed student photo path")
    return value
...
class EnrollmentCreate(BaseModel):
    ...
    status: str = Field(default="active", pattern="^(active|completed|withdrawn|transferred)$")
    notes: str | None = None
...
class EnrollmentUpdate(BaseModel):
    ...
    exit_date: date | None = None
    status: str | None = Field(default=None, pattern="^(active|completed|withdrawn|transferred)$")
    notes: str | None = None
