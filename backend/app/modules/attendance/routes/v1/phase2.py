...
from app.modules.attendance.services.operations import dashboard, list_sessions, session_completion
...
async def attendance_sessions(
    ...
    session_type: str | None = Query(None, min_length=1, max_length=64),
    session_status: str | None = Query(None, alias='status', min_length=1, max_length=32),
    class_level_id: UUID | None = Query(None),
    stream_id: UUID | None = Query(None),
    limit: int = Query(100, ge=1, le=200),
    tenant_id: UUID = Depends(read),
):
