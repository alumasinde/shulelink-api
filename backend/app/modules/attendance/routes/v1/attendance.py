...
from app.modules.attendance.schemas import (
    AttendanceMarkRequest, AttendanceMarkResult, AttendanceSessionCloseResponse,
    AttendanceSessionCreate, AttendanceSessionResponse, AttendanceStatusResponse, RosterStudent,
)
...
mark_perm = perm('attendance.mark')
sync_perm = perm('attendance.sync')
nemis_perm = perm('attendance.nemis')
    ...
    async with pool.acquire() as c:
        async with c.cursor() as cur:
            await cur.execute(
                ...
                'FROM attendance_statuses WHERE tenant_id=%s ORDER BY sort_order,code',
                (str(tenant_id),),
            )
...
async def session(session_id: UUID, tenant_id: UUID = Depends(read)):
    return await get_session(tenant_id, session_id)


@router.get('/sessions/{session_id}/roster', response_model=list[RosterStudent])
async def session_roster(session_id: UUID, tenant_id: UUID = Depends(read)):
    return await roster(tenant_id, session_id)
...
async def open_session(payload: AttendanceSessionCreate, principal=Depends(get_current_principal), tenant_id: UUID = Depends(manage)):
    session_id = await create_session(tenant_id, principal.user_id, payload.model_dump())
    return await get_session(tenant_id, session_id)
...
async def sync_batch(payload: dict, principal=Depends(get_current_principal), tenant_id: UUID = Depends(sync_perm)):
    return await receive_batch(
        tenant_id,
        str(payload.get('client_batch_id', '')),
        payload.get('items') or [],
        UUID(str(payload['device_id'])) if payload.get('device_id') else None,
    )


@router.post('/nemis/stage')
async def stage_nemis(attendance_date: date, academic_year_id: UUID | None = None, academic_term_id: UUID | None = None, tenant_id: UUID = Depends(nemis_perm)):
    return await stage_daily_attendance(tenant_id, attendance_date, academic_year_id, academic_term_id)
