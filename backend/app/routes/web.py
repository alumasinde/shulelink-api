...
from app.modules.attendance.routes.v1.attendance import router as attendance_router
from app.modules.attendance.routes.v1.phase_d import router as attendance_phase_d_router
from app.modules.attendance.routes.v1.phase2 import router as attendance_phase2_router
router = APIRouter(prefix=settings.api_v1_prefix)
router.include_router(health_router); router.include_router(auth_router); router.include_router(tenants_router); router.include_router(platform_academic_settings_router); router.include_router(platform_curriculum_router); router.include_router(tenant_context_router); router.include_router(school_structure_router); router.include_router(data_transfer_router); router.include_router(students_router); router.include_router(student_accounts_router); router.include_router(portal_router); router.include_router(student_portal_router); router.include_router(parent_portal_router); router.include_router(academics_router); router.include_router(curriculum_router); router.include_router(attendance_router); router.include_router(attendance_phase_d_router); router.include_router(attendance_phase2_router)
