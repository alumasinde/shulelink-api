...
async def get_tenant_pool(tenant_id: UUID) -> aiomysql.Pool:
    ...
    async with central.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT database_mode, database_host, database_port, database_name, database_user, database_password_encrypted, status FROM tenants WHERE id=%s",
                (str(tenant_id),),
            )
    ...
    try:
        ...
        # Ensure we don't leave a failed pool in the dict
        _pools.pop(key, None)
        raise
