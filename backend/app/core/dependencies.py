...
async def get_current_principal(
    ...
    if principal.user_type == "tenant" and principal.tenant_id is None:
        raise HTTPException(status_code=401, detail="Invalid tenant session")
    if principal.user_type == "platform" and principal.tenant_id is not None and not principal.tenant_access_session_id:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            ...
            await cur.execute(
                "SELECT user_type,user_id,tenant_id,revoked_at,expires_at "
                "FROM auth_sessions WHERE id=%s LIMIT 1",
            ...
            if (
                not session
                or session[0] != principal.user_type
                or str(session[1]) != str(principal.user_id)
                or (str(session[2]) if session[2] else None)
                != (str(principal.tenant_id) if principal.tenant_id else None)
                or session[3] is not None
                or session[4] <= now
            ):
                raise HTTPException(status_code=401, detail="Session is no longer active")

            if principal.user_type == "tenant":
                await cur.execute(
                    "SELECT 1 FROM tenant_memberships "
                    "WHERE tenant_id=%s AND tenant_user_id=%s AND status='active' LIMIT 1",
                    (str(principal.tenant_id), str(principal.user_id)),
                )
                if not await cur.fetchone():
                    raise HTTPException(status_code=403, detail="Tenant membership is no longer active")

            if principal.tenant_access_session_id:
                await cur.execute(
                    "SELECT tenant_id,expires_at,revoked_at "
                    "FROM tenant_access_sessions "
                    "WHERE id=%s AND platform_user_id=%s LIMIT 1",
                    (str(principal.tenant_access_session_id), str(principal.user_id)),
                )
                access = await cur.fetchone()
                if (
                    not access
                    or access[2] is not None
                    or access[1] <= now
                    or str(access[0]) != str(principal.tenant_id)
                ):
        ...
        async with pool.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    ...
                    "JOIN platform_role_permissions rp ON rp.platform_role_id=ur.platform_role_id "
                    "JOIN platform_permissions p ON p.id=rp.platform_permission_id "
                    "WHERE ur.platform_user_id=%s AND p.code=%s LIMIT 1",
...
async def get_tenant_id_from_host(request: Request) -> UUID:
    ...
    async with pool.acquire() as conn:
        async with conn.cursor() as cur:
            await cur.execute(
                "SELECT t.id,t.status FROM tenant_domains d "
                "JOIN tenants t ON t.id=d.tenant_id WHERE d.hostname=%s LIMIT 1",
                (hostname,),
            )
            row = await cur.fetchone()
...
async def require_tenant(
    ...
    tenant_id: UUID = Depends(get_tenant_id_from_host),
):
        ...
        async with central.acquire() as conn:
            async with conn.cursor() as cur:
                await cur.execute(
                    ...
                    "WHERE m.tenant_id=%s AND m.tenant_user_id=%s AND m.status='active' "
                    "AND p.code=%s LIMIT 1",
