...
logger = logging.getLogger("shulelink.database")
...
async def run_migrations() -> None:
    """Run pending migrations with distributed lock support."""
    pool = get_central_pool()
            ...
            async with connection.cursor() as cursor:
                ...
                for path in files:
                    try:
                        ...
                        logger.warning(f"Skipping migration file with invalid version: {path.name}")
                        continue
                    ...
                    if version in seen_versions:
                        raise RuntimeError(f"Duplicate unapplied migration version detected: {version}")
                    ...
                    try:
                        ...
                        applied.add(version)
                        logger.info(f"Migration applied successfully: {path.name}")
                    except Exception as exc:
                        logger.error(f"Failed to apply migration {path.name}: {exc}")
                        raise
